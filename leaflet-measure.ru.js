! function(e) {
	...
	L.Control.Measure = L.Control.extend({
		...
		_initLayout: function() {
			...
			this.$startPrompt = (0, c.selectOne)(".js-startprompt", t), this.$measuringPrompt = (0, c.selectOne)(".js-measuringprompt", t), this.$startHelp = (0, c.selectOne)(".js-starthelp", t), this.$results = (0, c.selectOne)(".js-results", t), this.$measureTasks = (0, c.selectOne)(".js-measuretasks", t), this._collapse(), this._updateMeasureNotStarted(), L.Browser.android || (L.DomEvent.on(t, "click", this._expand, this), L.DomEvent.on(t, "click", this._collapse, this)), L.DomEvent.on(r, "click", L.DomEvent.stop), L.Browser.touch ? L.DomEvent.on(r, "click", this._expand, this) : L.DomEvent.on(r, "focus", this._expand, this), L.DomEvent.on(n, "click", L.DomEvent.stop), L.DomEvent.on(n, "click", this._startMeasure, this), L.DomEvent.on(o, "click", L.DomEvent.stop), L.DomEvent.on(o, "click", this._finishMeasure, this), L.DomEvent.on(i, "click", L.DomEvent.stop), L.DomEvent.on(i, "click", this._handleMeasureDoubleClick, this)
		},
		_expand: function() {
		...
		_finishMeasure2: function() {
			var e = L.extend({}, this._resultsModel, {
				points: this._latlngs
			});
			this._locked = !1, L.DomEvent.off(this._container, "mouseover", this._handleMapMouseOut, this), this._clearMeasure(), this._captureMarker.off("mouseout", this._handleMapMouseOut, this).off("dblclick", this._handleMeasureDoubleClick, this).off("click", this._handleMeasureClick, this), this._map.off("mousemove", this._handleMeasureMove, this).off("mouseout", this._handleMapMouseOut, this).off("move", this._centerCaptureMarker, this).off("resize", this._setCaptureMarkerIcon, this), this._layer.removeLayer(this._measureVertexes).removeLayer(this._captureMarker), this._measureVertexes = null, this._updateMeasureNotStarted()
		},
		_clearLyr: function() {
			var th = this
			$.each(th._layer._layers, function(k,l) {
		 		th._layer.removeLayer(l)
			})
		},
	...
	e.exports = '<a class="{{ model.className }}-toggle js-toggle" href=# title="Измерить площадь">Измерение</a> <div class="{{ model.className }}-interaction js-interaction"> <div class="js-startprompt startprompt"> <h5>Измерить площадь</h5> <ul class=tasks> <a href=# class="js-start start">Добавить</a> </ul> </div> <div class=js-measuringprompt> <h5>Измерить площадь</h5> <p class=js-starthelp>Для начала измерения добавьте точку на карту</p> <div class="js-results results"></div> <ul class="js-measuretasks tasks"> <li><a href=# class="js-cancel cancel">Отмена</a></li> <li><a href=# class="js-finish finish">Завершить</a></li> </ul> </div> </div> '
}, function(e, t, r) {
	e.exports = '<div class=group> <p class="lastpoint heading">Последняя точка</p> <p>{{ model.lastCoord.dms.y }} <span class=coorddivider>/</span> {{ model.lastCoord.dms.x }}</p> <p>{{ numberFormat(model.lastCoord.dd.y, 6) }} <span class=coorddivider>/</span> {{ numberFormat(model.lastCoord.dd.x, 6) }}</p> </div> <% if (model.pointCount > 1) { %> <div class=group> <p><span class=heading>Периметр</span> {{ model.lengthDisplay }}</p> </div> <% } %> <% if (model.pointCount > 2) { %> <div class=group> <p><span class=heading>Площадь</span> {{ model.areaDisplay }}</p> </div> <% } %> '
}, function(e, t, r) {
	e.exports = '<h5>Точка</h5> <p>{{ model.lastCoord.dms.y }} <span class=coorddivider>/</span> {{ model.lastCoord.dms.x }}</p> <p>{{ numberFormat(model.lastCoord.dd.y, 6) }} <span class=coorddivider>/</span> {{ numberFormat(model.lastCoord.dd.x, 6) }}</p> <ul class=tasks> <li><a href=# class="js-zoomto zoomto">Увеличить</a></li> <li><a href=# class="js-deletemarkup deletemarkup">Удалить</a></li> </ul> '
}, function(e, t, r) {
	e.exports = '<h5>Линия</h5> <p>Длинна: {{ model.lengthDisplay }}</p> <ul class=tasks> <li><a href=# class="js-zoomto zoomto">Увеличить</a></li> <li><a href=# class="js-deletemarkup deletemarkup">Удалить</a></li> </ul> '
}, function(e, t, r) {
	e.exports = '<h5>Участкок</h5> <p>Площадь: {{ model.areaDisplay }}</p> <p>Периметр: {{ model.lengthDisplay }} </p> <ul class=tasks> <li><a href=# class="js-zoomto zoomto">Увеличить</a></li> <li><a href=# class="js-deletemarkup deletemarkup">Удалить</a></li> </ul> '
}]);
